...
public class SplineFollower : SplineTracer {
    ...
    /// <summary>
    /// If the follow mode is set to Uniform and there is an added offset in the motion panel, this will preserve the uniformity of the follow speed
    /// </summary>
    [HideInInspector] public bool preserveUniformSpeedWithOffset = false;
    ...
    [SerializeField] [HideInInspector] float _followSpeed = 1f;
    [SerializeField] [HideInInspector] float _followDuration = 1f;
    ...
    void Follow() {
        switch (followMode) {
            ...
            case FollowMode.Time:
                if (_followDuration == 0.0)
                    Move(0.0);
                else
                    Move((double) Time.deltaTime / _followDuration);
                break;
        }
    }

    public void Restart(double startPosition = 0.0) { SetPercent(startPosition); }
